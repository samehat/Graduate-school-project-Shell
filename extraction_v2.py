...
def evaluate_expression_v2(expr, *v):
    ...
    for i, val in enumerate(v[:10]):                 # v = vpp,vnn,vpn
        expr = expr.replace(f"vpp[{i}]", str(val))
    for i, val in enumerate(v[10:20]):                 # v = vpp,vnn,vpn
        expr = expr.replace(f"vpp[{i}]", str(val))
    for i, val in enumerate(v[20:]):                 # v = vpp,vnn,vpn
        expr = expr.replace(f"vpp[{i}]", str(val))
    # Evaluate the resulting expression
    return eval(expr)
...
#v_test= np.array([-2321,-1524,-937,-700,-160,-447,140,-640,241,-1752]) - np.array([-2321])

#a,b,c=curv_fit_function(fit_list)
#v_fin = [0.0, 1132.8482748289173, 1419.08803204841, 1543.1294891098823, 2335.7589015211815, 1765.757607051619, 2458.424225032527, 1694.8994411358872, 2593.7412986206, 820.0078985374051]
#v_prof = [0,1714,1487,1925,2352,2053,2528,2247,2656,820]
#loss,E_calc,E_exp = diagonalisation([[8,1]],v_prof)
#print(E_calc)


mat = dict_th_ph[(str(9),str(9),str(9.0))]
